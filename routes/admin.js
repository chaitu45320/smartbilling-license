...
router.post('/keys/generate', (req, res) => {
  ...
  const tDays    = type === 'trial' ? (parseInt(trialDays) || 10) : null;

  db.run(`INSERT INTO license_keys (key_hash,key_display,type,email,max_devices,is_active,issued_at,notes,trial_days)
          VALUES (?,?,?,?,?,1,?,?,?)`,
    [keyHash, key, type, email.trim().toLowerCase(), max, Date.now(), notes || '', tDays]);

  console.log(`[KEY GENERATED] ${key} | ${type} | ${email}${tDays ? ' | ' + tDays + ' days' : ''}`);
...
router.get('/keys', (req, res) => {
  ...
  const result = keys.map(k => ({
    ...k,
    daysLeft:    getDaysLeft(k.issued_at, k.type, k.trial_days),
    isExpired:   k.type === 'trial' && isTrialExpired(k.issued_at, k.trial_days),
...
router.get('/keys/:hash', (req, res) => {
  ...
  return res.json({
    success: true,
    key: { ...key, daysLeft: getDaysLeft(key.issued_at, key.type, key.trial_days), isExpired: key.type === 'trial' && isTrialExpired(key.issued_at, key.trial_days) },
    devices,
    logs
  });
});
