...
const TRIAL_DAYS = 10; // default fallback only
...
function getDaysLeft(activatedAt, type, trialDays) {
  ...
  return Math.max(0, Math.ceil((activatedAt + days * 86400000 - Date.now()) / 86400000));
}

function isTrialExpired(activatedAt, trialDays) {
  const days = trialDays || TRIAL_DAYS;
  return Date.now() > activatedAt + days * 86400000;
}
